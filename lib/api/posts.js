...
const getSlugs = buildGetSlugs('posts')

const readPostFile = buildReadMarkdownFile('posts', async ({
  content,
  frontmatter,
  slug,
}) => {
...
export async function getAllPosts() {
  return (await Promise.all(getSlugs().map(getPostBySlug))).sort((a, b) => new Date(b.date) - new Date(a.date))
}
