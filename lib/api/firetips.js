...
const readFiretipFile = buildReadMarkdownFile('firetips', async ({
  content,
  frontmatter,
  slug,
}) => {
...
export async function getAllFiretips() {
  return (await Promise.all(getSlugs().map(readFiretipFile))).sort((a, b) => new Date(b.date) - new Date(a.date))
}
